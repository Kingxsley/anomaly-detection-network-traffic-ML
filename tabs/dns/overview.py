...
def render(time_range, time_range_query_map):
    ...
    # Get the query duration based on the selected time range
    query_duration = time_range_query_map.get(time_range, "-24h")
    
    # Load data from SQLiteCloud based on the selected time range
    df = load_predictions_from_sqlitecloud(time_window=query_duration)

    if not df.empty:
        # Basic statistics for the overview
        ...
        # Attack summary - avg, max, min reconstruction error
        ...
        # Top Source IPs with most attacks
        ...
        # Top Destination IPs with most attacks
