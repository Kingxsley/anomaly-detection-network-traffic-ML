...
INFLUXDB_URL = st.secrets.get(f"{'DOS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'INFLUXDB'}_URL", "")
INFLUXDB_ORG = st.secrets.get("INFLUXDB_ORG", "")
INFLUXDB_BUCKET = st.secrets.get(f"{'DOS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'INFLUXDB'}_BUCKET", "")
INFLUXDB_TOKEN = st.secrets.get(f"{'DOS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'INFLUXDB'}_TOKEN", "")
SQLITE_HOST = st.secrets.get(f"{'DOS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'SQLITE'}_HOST", "")
SQLITE_PORT = int(st.secrets.get(f"{'DOS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'SQLITE'}_PORT", 8860))
SQLITE_DB = st.secrets.get(f"{'DOS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'SQLITE'}_DB", "")
SQLITE_APIKEY = st.secrets.get(f"{'DOS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'SQLITE'}_APIKEY", "")

# --- Discord Alert ---
def send_discord_alert(result):
    message = {
        "content": (
            f"\U0001f6a8 **{'DoS' if st.secrets.get('DASHBOARD_TYPE') == 'DOS' else 'DNS'} Anomaly Detected!**\n"
...
def log_to_sqlitecloud(record):
    try:
        ...
        """, (
            ...
            "DNS" if st.secrets.get('DASHBOARD_TYPE') == "DNS" else "DoS",
...
def get_data(api_url):
    try:
        ...
        with InfluxDBClient(url=INFLUXDB_URL, token=INFLUXDB_TOKEN, org=INFLUXDB_ORG) as client:
            ...
            |> range(start: -5m)
            |> filter(fn: (r) => r._measurement == "{'dns' if st.secrets.get('DASHBOARD_TYPE') == 'DNS' else 'dos'}")
        ...
        st.warning(f"Failed to fetch live data from InfluxDB: {e}")
        return []

# --- Get Historical Data ---
@st.cache_data(ttl=600)
def get_historical(start, end, dashboard_type):
    try:
        if not INFLUXDB_URL:
            raise ValueError("No host specified.")
        with InfluxDBClient(url=INFLUXDB_URL, token=INFLUXDB_TOKEN, org=INFLUXDB_ORG) as client:
            ...
            |> filter(fn: (r) => r._measurement == "{'dns' if dashboard_type == 'DNS' else 'dos'}")
            |> filter(fn: (r) => r._field == "inter_arrival_time" or r._field == "dns_rate")
            |> pivot(rowKey: ["_time"], columnKey: ["_field"], valueColumn: "_value")
            |> sort(columns: ["_time"], desc: false)
            '''
