...
def render(time_range, time_range_query_map):
    ...
    st.title(f"{st.secrets.get('DASHBOARD_TYPE', 'DNS')} Anomaly Detection Overview")
    ...
    if st.secrets.get("DASHBOARD_TYPE", "DNS") == "DNS":
        ...
        query_duration = time_range_query_map.get(time_range, "-24h")
        df = load_predictions_from_sqlitecloud(time_window=query_duration)  # This can be adjusted for DOS-specific data fetching
