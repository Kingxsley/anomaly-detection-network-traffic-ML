...
df = get_historical(start_date, end_date).head(100)
if not df.empty:
    ...
    st.dataframe(df.style.apply(highlight_hist, axis=1))
    ...
    st.download_button("Download CSV", df.to_csv(index=False), file_name="historical_dns_data.csv")
else:
