...
def render(thresh, highlight_color):
    ...
    if not df.empty:
        ...
        # Apply highlight color to the columns dynamically using map()
        def highlight_attack_columns(row):
            return [f'background-color: {highlight_color}' if row['label'] == "Attack" else '' for _ in row]

        st.dataframe(df_view.style.apply(highlight_attack_columns, axis=1))
